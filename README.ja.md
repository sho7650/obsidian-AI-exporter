...
- **ファイル名衝突対策**: 別の会話に属するノートを絶対に上書きせず、途中までしか読み込めなかった内容で完全なノートを置き換えることもしない
...
- **入れ子のコードブロック**: コードブロックを含むコードブロックも内容に合わせてフェンスを調整するため、Markdown について書かれた回答もそのまま保存
- **メッセージ形式**: Obsidian コールアウト（デフォルト）／引用ブロック／プレーンテキストから選択可能。`[!QUESTION]` `[!NOTE]` のコールアウト種別も話者ごとに設定できます
- **YAML フロントマター**: タイトル、ソース、URL、日時、タグなどのメタデータを自動生成。タグ名は `{platform}` トークン付きで設定でき、Vault の既存タグに合わせられます
...
> **非常に長い会話では時間が足りないことがあります。** 自動スクロールは「15 秒進捗がない」か「合計 300 秒」で打ち切られます。途中で止まった旨の警告が出たら、詳細設定の**自動スクロールのアイドル待機時間**または**自動スクロールの上限時間**を大きくして同期し直してください（どちらに達したかは警告文に出ます）。 ノート本文は既定で 8 MiB までです。「above the … limit」で同期が失敗したら、詳細設定の**ノートの最大サイズ**（最大 16 MiB）を大きくしてください。
...
詳細設定の**メッセージ形式**で、上記のコールアウト／引用ブロック／プレーンテキストを切り替えられます。**ユーザーコールアウトタイプ**と**アシスタントコールアウトタイプ**で `QUESTION` / `NOTE` のキーワードを変更できます。

**会話のタグ**と **Deep Research のタグ**（詳細設定 → フロントマター項目）で `tags:` の内容を、既に Vault で使っているタグに合わせられます。いずれもカンマ区切りで、`{platform}` はソース id（`gemini`、`claude`、`chatgpt`、`perplexity`、`notebooklm`）に置き換わり、`ai/chat` のような階層タグも使えます。空欄にすると既定値（`ai-conversation, {platform}` と `ai-research, deep-research, {platform}`）に戻ります。各項目は Obsidian のタグ規則（文字・数字・`_`・`-`・`/`、空白不可）に従い、不正な項目が 1 つでもあると欄全体が既定値に戻ります。既存ノートは書き換えません。追記モードでも `tags:` ブロックはそのまま残ります。

コードブロックのフェンスは内容に合わせて長さを決めます。`README.md` やプロンプトを AI に書かせたときのように、ブロックの中にさらにフェンス付きブロックが含まれる場合、外側のフェンスをバッククォート 4 つ以上に伸ばして内側のフェンスを内容として保持します（Obsidian が案内しているネストの書き方と同じです）。元のフェンス文字は復元できない（ページにはレンダリング後のコードしか存在しない）ため、外側は常にバッククォートになります。
...
また、自動スクロールが途中で打ち切られたエクスポートは、vault にある既存ノートの方がメッセージ数が多い場合、書き込まずに中止します。何も上書きされず、両方の件数がメッセージに表示されるので、途中までの取得が完全なノートを置き換えることはありません。
...
- 生成画像のダウンロードは画像エクスポート有効時のみ、対象はエクスポート中の会話に含まれる画像のみです。Gemini の画像は Google の画像 CDN（`googleusercontent.com`、`lh3.google.com` にリダイレクトされます）から、ChatGPT の画像は `chatgpt.com` 自身から取得します — いずれも AI のページが既に画像を読み込んでいるのと同じホストです
